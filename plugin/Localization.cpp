...
const std::unordered_map<std::string, StringPair>& stringTable() {
    static const std::unordered_map<std::string, StringPair> table{
        ...
        {"clip.statusPending", StringPair{u8"待分離", "Pending"}},
        {"clip.statusSeparating", StringPair{u8"分離中…", "Separating..."}},
        {"clip.statusDone", StringPair{u8"完成", "Done"}},
        {"clip.statusFailed", StringPair{u8"失敗", "Failed"}},
        {"clip.importedCountPrefix", StringPair{u8"已匯入 ", "Imported "}},
        {"clip.importedCountSuffix", StringPair{u8" 個檔案，按「分離」開始處理", " files - press Separate to start"}},
        {"clip.batchSeparationFinished", StringPair{u8"全部分離完成", "All clips separated"}},
        {"clip.exportFinishedPrefix", StringPair{u8"已匯出 ", "Exported "}},
        {"clip.exportFinishedSuffix", StringPair{u8" 個檔案", " files"}},
        ...
        {"filechooser.importMediaTitle",
         StringPair{u8"匯入音訊或影片檔案", "Import an audio or video file"}},
        {"filechooser.exportVocalsTitle", StringPair{u8"匯出人聲", "Export vocals"}},
        {"filechooser.exportAccompanyTitle",
         StringPair{u8"匯出伴奏", "Export accompaniment"}},
        {"alert.importMediaFirstTitle",
         StringPair{u8"請先匯入媒體", "Import media first"}},
        ...
        {"alert.defaultModelMissingTitle",
         StringPair{u8"缺少預設模型", "Default model is missing"}},
        {"alert.defaultModelMissingMessage",
         StringPair{u8"一般面板需要 htdemucs 模型。請重新執行安裝程式，或開啟進階面板下載該模型。",
                    "The general panel requires the htdemucs model. Re-run "
                    "the installer or open the Advanced panel to install "
                    "it."}},
        {"alert.nothingToExportTitle",
         StringPair{u8"沒有可匯出的內容", "Nothing to export"}},
        {"alert.nothingToExportMessage",
         StringPair{u8"請先匯入或錄製音訊，並執行分離後再匯出。",
                    "Import or record audio, then run Separate before "
                    "exporting."}},
        {"dialog.exportStemsOrMixTitle",
         StringPair{u8"匯出 HTDemucs 音軌或混音", "Export HTDemucs stems or mix"}},
        {"roformer.tagExperimental",
         StringPair{u8" 【實驗性】", " [Experimental]"}},
        {"roformer.tagAudited", StringPair{u8" 【已審核】", " [Audited]"}},
        {"roformer.noMatchingModels",
         StringPair{u8"沒有符合的模型", "No matching models"}},
        {"roformer.unknownModel", StringPair{u8"未知模型", "Unknown model"}},
        {"roformer.statusExperimental", StringPair{u8"實驗性", "Experimental"}},
        {"roformer.statusAudited", StringPair{u8"已審核", "Audited"}},
        {"roformer.statusDownloaded", StringPair{u8"已下載", "Downloaded"}},
        {"roformer.statusNotDownloaded",
         StringPair{u8"未下載", "Not downloaded"}},
        ...
        {"status.roformerStartingEngine",
         StringPair{u8"正在啟動分離引擎（首次啟動較慢）…",
                    "Starting the separation engine (slower on first run)..."}},
        {"status.roformerPreparing",
         StringPair{u8"正在準備模型…", "Preparing the model..."}},
        {"status.roformerDownloading",
         StringPair{u8"正在下載模型 ", "Downloading "}},
        {"status.roformerLoadingModel",
         StringPair{u8"正在載入模型到運算裝置：", "Loading onto the device: "}},
        {"status.roformerSeparating", StringPair{u8"分離中", "Separating"}},
        {"status.roformerRemainingPrefix",
         StringPair{u8"剩餘約 ", "about "}},
        {"status.roformerVerifying",
         StringPair{u8"正在驗證輸出…", "Verifying the output..."}},
        {"status.secondsSuffix", StringPair{u8" 秒", "s"}},
        {"status.minutesSuffix", StringPair{u8" 分 ", "m "}},
        ...
        {"status.startingDemucsWorker",
         StringPair{u8"正在準備分離…", "Preparing to separate..."}},
