...
- SIGN (macos, 文件驗證): 本機建置的 `.app` **沒有 quarantine 旗標**，所以雙擊直接開、
  ...
  **可轉用的教訓：驗證「某個東西應該被擋住」時，第一個要問的不是「它被擋住了嗎」，
  而是「我的檢查方法看得見它沒被擋的那種情況嗎」。** 一個只會回報「找不到」的檢查，
  對「被擋住」和「跑在我沒想到的地方」會給出一模一樣的答案，而那兩件事的結論相反。
  同理，驗證前要先排除自己製造的污染：我第一次實驗前已經成功開過同一個 App，
  那次可能留下核可紀錄，所以第二次必須刪掉重裝、並在**第一次開啟之前**就加上旗標。
  Finder 雙擊是否與 `open(1)` 一致仍未驗（需要 GUI）。所以**給使用者的文件不要斷言會被擋**，
  要寫成「如果被擋…如果直接開起來就跳過」，兩種結果都讓他知道自己在正軌上。
- SIGN (macos): **x86 的凍結尾段不能用 arm64 的時間感判斷。** PyInstaller 印出
  `Build complete!` 之後，腳本還要執行三次 worker（`--help`、`roformer --help`、`--import-check`），
  而 x86 在 Rosetta 下**每一次**都要付翻譯成本，所以尾段比 arm64 長好幾分鐘。log 停在
  `Build complete!` 看起來像中斷，其實還在驗證——本輪我因此誤判成「凍結失敗」，
  去查 `runtime-manifest.json` 發現不存在，但幾分鐘後它就出現了。
  **這正是同一個坑的兩面**：把完成標記定成「執行檔存在」會讓人**太早說成功**
  （拿到還沒通過 import-check 的 dist），定成 manifest 則可能讓人**太早說失敗**。
  後者無害（再等就好），前者會出貨壞東西，所以 manifest 是對的選擇——但查之前要先等夠久。
- SIGN (2026-09-15): **把某個數字修好之後，要回頭找所有依那個數字寫成的文字。** 本輪把 x86 的
  系統下限從 14.0 壓到 12.0（改用 `scipy<1.14`，其預編 `.so` 是 10.9 而非 14.0），但給測試者的
  文件裡還留著依舊值寫的「macOS 小於 14 請停下來告訴我」——那句話會把一台 macOS 12、
  **實際上跑得起來**的 Intel Mac 擋在門外。修了程式沒回頭改文件，症狀不是「壞掉」而是
  「白白失去可用的機器」，而且沒有任何錯誤訊息會提醒你。
- SIGN (macos): 判斷一個 bundle 的真實系統下限，**要掃全部 Mach-O 並列出「最高的那幾個是哪些檔案」**，
  不能只回答「最高值是多少」。本輪 x86 壓到 12.0 之後，剩下最高的恰好是
  `Contents/MacOS/<主執行檔>` 與我們自己建的 `ffmpeg`／`ffprobe` 三個——
  **這代表沒有任何第三方 wheel 留在 12 以上，下限完全由我們自己控制**。
  那比「14.0 歸零」這個數字強：它同時證明結果不會被某個看不見的相依悄悄拉高。
  掃描指令見本檔前述條目（`otool -l` 取 `LC_BUILD_VERSION` 的 `minos`；
  **wheel 檔名的 `macosx_X_Y` tag 是 pip 的相容性標籤，不等於 `LC_BUILD_VERSION`，不可當答案**）。
- SIGN (2026-09-15, 通則): **驗證工具裡的「預設值」本身就是缺陷來源**——它讓工具在「沒有被正確
  告知要驗什麼」的情況下，仍然給出一個**有自信的答案**。同一天撞到兩個同形狀的例子：
  Windows 端的 `check_public_repo.ps1`／`verify_windows_web_packages.ps1` 版號預設 `0.1.0`，
  而 `dist\windows-web` 留著歷來每個版本，所以不給版號就**對著一組舊檔案回報 PASS**；
  我這邊用 `pgrep -f "<原路徑>"` 判斷 App 有沒有啟動，而 App Translocation 把它搬到別的路徑，
  於是**對錯的路徑回報「沒啟動」**。兩者都不是「檢查失敗」，是**檢查回答了另一個問題**。
  所以：驗證工具的關鍵參數應該**必填**而不是給預設；而寫檢查時要先問
  「我的方法看得見我不預期的那種情況嗎」。
- SIGN (macos): `.app` 宣告的 `CMAKE_OSX_DEPLOYMENT_TARGET` 要**等於 bundle 內實測的最高 minos**，
  而兩個架構的下限不同（x86_64 是 12.0＝主執行檔與我們自建的 ffmpeg；arm64 是 14.0＝torch 的 wheel），
  所以那個值必須**跟著 `--runtime-arch` 走**，不能取單一數字：取 14 會讓 Intel 版白白放棄
  Monterey 的涵蓋，取 12 則讓 arm64 版對 macOS 12/13 開出空頭支票。
  宣告過低的症狀不是「擋不住」而是**擋得太晚**：App 開得起來、吃得下檔案，然後在 worker 載入
  torch 時才死，看起來像「分離功能壞了」而不是「你的系統太舊」。宣告正確之後改成啟動時就被
  dyld 拒絕——**這是行為改變，不只是文件修正**。
- SIGN (2026-09-15, 發行): **`hf upload` 在沒有主控台的背景行程裡會無聲卡死**——1 個執行緒、
  CPU 0%、I/O 只有約 1 KB，**沒有任何錯誤訊息**，從外面看跟「正在慢慢傳」完全一樣，
  要去看行程狀態才會發現它根本沒在動（推測是進度條／終端機互動相關）。
  改用 Python API 就正常：
  ```python
  from huggingface_hub import HfApi
  HfApi().upload_file(path_or_fileobj=..., path_in_repo=..., repo_id=..., repo_type="model")
  ```
  這是 Windows 端先踩到並告知的。**通則：在背景行程裡呼叫會畫進度條的 CLI，
  失敗方式往往是「看起來還在跑」而不是報錯**；長時間沒有位元組進出就該去看行程狀態，
  不要只看「程序還在」。
