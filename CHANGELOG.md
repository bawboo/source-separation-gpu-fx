...
- **分離模式與音軌推桿都加上中文意思**，例如 `Vocals（人聲）`、`Dereverb（去混響）`、
  `Residual（其餘）`。模型圈子用的是英文名稱，App 也照樣顯示，但那些名稱對中文使用者
  沒有說明效果。英文介面維持純英文名稱；中文只是附加而非取代，所以仍看得到模型自己的
  命名。目錄新增分類若還沒有對照，會原樣顯示而不會變成壞掉的標籤
- **快速匯出（僅匯出人聲／伴奏）改為套用輸出增益**，與預覽播放一致——先前預覽會套用而
  匯出不會，所以聽到的音量不等於匯出的音量。音軌推桿不納入（單軌匯出不是那些推桿描述的
  混音）；輸出增益維持預設 0 dB 時，匯出結果與先前完全相同。逐軌匯出仍維持原始音量不變
