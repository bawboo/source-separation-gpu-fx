...
- 新增 `htdemucs_format_matrix_check`：對檔案選擇器接受的每一種格式（WAV／FLAC／
  ...
  MKV、影片比音訊長、0.05 秒短檔、20 分鐘的長檔（GPU 分離 59 秒），以及匯出到不存在的
  磁碟機（必須報錯、不得卡住、之後仍能正常匯出）；`--batch N` 可把批次段落放大到
  12 個檔案。同一套檢查也對 CPU 凍結 runtime 跑過（4 種格式 49/49）。測試資料為 30 秒合成訊號，由腳本產生，不需要真實歌曲
