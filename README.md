...
內建兩個模型家族，每種分離只提供一個經過驗證的代表性模型，不用在幾十個
相似權重之間猜：

- **HTDemucs** — 4 軌（鼓／貝斯／其他／人聲）與 6 軌（再加吉他／鋼琴）
- **MelBand RoFormer** — 人聲、伴奏、卡拉 OK、吉他、去混響、去噪、串音抑制、
  群聲、氣音，共 8 類 9 個模型

CPU 版只列出實測在一般電腦上跑得完的模型：HTDemucs 4／6 軌與 RoFormer 吉他
（其餘 RoFormer 模型在 CPU 上一首歌要 40 分鐘到數小時，不提供）；GPU 版提供全部。

## 怎麼用

1. **匯入** 一個或多個音訊／影片檔（按「匯入」，或直接把檔案拖進視窗）
2. **選模式** —— 先決定「要分什麼」，每個模式都帶好預設模型，也可以換成同類的其他模型
3. **分離** —— 逐一處理，先完成的可以先試聽
4. **調整** 各軌音量，**匯出** 成 WAV（或把新音軌回填回原本的 MP4）；匯出完成後按
   「開啟輸出位置」直接在檔案總管看到結果
...
- **鍵盤快捷鍵**：空白鍵試聽／暫停、Esc 取消、Ctrl+O 匯入、Ctrl+E 匯出（進階面板）
...
| 類型 | 副檔名 |
|---|---|
| 音訊 | WAV、FLAC、AIFF（.aif／.aiff）、MP3、OGG（Vorbis／Opus）、M4A（AAC） |
| 影片 | MP4、MOV、MKV、AVI、WebM、M4V、WMV、MPEG |

任何取樣率、位元深度與聲道數都可以（8～32 位元、單聲道到 5.1，22.05～96 kHz），
匯入時一律轉成 44.1 kHz 立體聲。影片只取音軌；「回填 MP4」會保留原本的影片畫面，
H.264／HEVC／MPEG-4 等 MP4 支援的編碼直接複製，WebM（VP8/VP9）、WMV 這類 MP4 裝不下的
編碼會自動重新編碼成 H.264，狀態列會顯示「正在重新編碼影片」。

檔案完整路徑超過 260 字元時 Windows 無法開啟，App 會提示把檔案移到較短的資料夾。
